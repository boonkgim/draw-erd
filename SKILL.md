...
`docs/<same-folder>/NN-erd.html`, carrying **the same `NN` as the `NN-data-model.md` it
renders** — a view of that document, not new material, so it does not take the next free
number in the folder's sequence the way a new document would under `design-db`'s rule.
Redrawing the same model overwrites the same file. Do not commit — that decision belongs to
